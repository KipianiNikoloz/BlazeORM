...
- Use `session.bulk_create(instances)` for atomic, ordered creation with generated keys and normal lifecycle behavior.
- `DateTimeField(auto_now_add=True)` records creation time, `auto_now=True` advances on every save, and `session.refresh(instance)` discards local scalar changes in favor of database state.
